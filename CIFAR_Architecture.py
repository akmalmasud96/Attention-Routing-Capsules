...
class AR_CapsNet_CIFAR(nn.Module):
   
   def __init__(self,input_shape,args):
       super(AR_CapsNet_CIFAR, self).__init__()
       ...
       self.BN2 = Conv2d_bn(input_shape = [None,64,32,32],filters=64,kernel_size=3, strides=1, padding='same')

       ## Primary Capsules
       self.Primary_Cap = PrimaryCap( input_shape = [None,64, 32,32], n_channels = 8, dim_capsule = dim_caps, decrease_resolution = True)

       ## Convolutional Capsules
       if self.layernum == 0:
         self.FullyConvCaps = FullyConvCaps(input_shape = [None, 8, 16, 16, 16],n_channels = 10,dim_capsule=dim_caps)
        
       elif self.layernum == 1:
         self.ConvCaps = ConvCaps(input_shape = [None, 8, 16, 16, 16],n_channels=8, dim_capsule=dim_caps, decrease_resolution = True)
         self.FullyConvCaps = FullyConvCaps(input_shape = [None, 8, 16, 8, 8],n_channels = 10,dim_capsule=dim_caps)

       elif self.layernum == 2:
         self.ConvCaps_2 = Add_ConvCaps(input_shape = [None, 8, 16, 16, 16], layer_num = 2,dim_capsule = dim_caps)
         self.FullyConvCaps = FullyConvCaps(input_shape = [None, 8, 16, 8, 8],n_channels = 10,dim_capsule=dim_caps)

       elif self.layernum == 3:
         self.ConvCaps_3 = Add_ConvCaps(input_shape = [None, 8, 16, 16, 16], layer_num = 3,dim_capsule = dim_caps)
         self.FullyConvCaps = FullyConvCaps(input_shape = [None, 8, 16, 8, 8],n_channels = 10,dim_capsule=dim_caps)
